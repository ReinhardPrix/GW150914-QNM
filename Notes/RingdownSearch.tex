...
However the numbers and posterior contours are different.
This version is more mature/converged, and more fully tested and characterized, after a number of bugs and possible inconsistencies have been fixed
since v3 (and even v4-).
We propose to update the text and results when re-submitting the paper to PRL in the following way:

\begin{center}
  \parbox{0.7\textwidth}{
    We use data band-passed to ${[20,1900]}\,\Hz$\\
    \ldots\\
    ...
    becomes undetectable at about $\tStart \gtrsim \tMerger + 8.5\,\ms$,
    where $B \lesssim 1$.
  }
\end{center}
...
\includegraphics[width=1\textwidth]{{\onSourceDir/PSDs}.pdf}\\
\includegraphics[width=1\textwidth]{{\onSourceDir/W-OW-Spectra}.pdf}
...
\parbox{\textwidth}{
  \includegraphics[width=0.9\textwidth]{{\InjSignalOnlyPriorRangesDir/Injections-PE-errors}.pdf}\\
  \includegraphics[width=0.9\textwidth]{{\InjSignalOnlyPriorRangesDir/Injections-PE-coverage}.pdf}
}
...
\newpage
\subsubsection{Case 1(ii): IMR $\{\freq,\tau\}$ injections, signal-only with assumed white noise}
\label{sec:case-1:injections-signal-only}
...
\newpage
\subsubsection{Case 2(ii): IMR $\{\freq,\tau\}$ injections, \textbf{known} Gaussian white noise}
\label{sec:case-2:injections-known-Gauss}
...
\newpage
\subsubsection{Case 3(ii): IMR $\{\freq,\tau\}$ injections, \textbf{unknown} Gaussian white noise}
\label{sec:case-3:injections-unknown-Gauss}
...
\newpage
\subsubsection{Case 4(ii): IMR $\{\freq,\tau\}$ injections, off-source detector data}
\label{sec:case-4:injections-real-noise}
...
\subsection{Varying $\tStart$ at fixed IMR injection $\{A = 2.5\times10^{21}, \phi_0 = 0\}$}
