...
A number of (small) changes compared to the previous (v3,v3+) versions, which were used in the submitted paper (\href{http://arxiv.org/abs/1602.03841}{arXiv:1602.03841}).
...
\item expanded narrowband data-range to $[30, 1900]\,\Hz$ (avoid lower and upper boundaries near 10Hz and 2000Hz respectively, where pwelch() psd estimation trails off).
...
\subsection{Results on GW150914}
...
We show results for QNM start-times $\tStart - \tMerger \in \{1,\,3,\,5,\,7\}\,\ms$ (referring to H1 arrival times).
...
\newpage
\section{Characterizing the search performance}

We perform two tests to characterize the performance of the search pipeline and parameter estimation (PE).
First, we test the distribution of Bayes factors, SNR and PE-intervals on ``off course'' noise data, in order to assess the significance of the ``on
source'' values found for GW150914.
Second, we perform searches on QNM-injections with known parameters to quantify the accuracy of parameter-estimation and of the posterior coverage.

\subsection{Off-source searches}
\label{sec:source-searches}

We test off-source search performance on Gaussian white noise, as well as on real detector data around GW150914 (using both the intervals
$\tMerger + [-3,-0.5]\,\sec$ and $\tMerger + [0.5,3]\,\sec$).
